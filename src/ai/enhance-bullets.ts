import type { WorkExperience, Project, EnhancedBullet } from '../types/resume.js'
...
export async function enhanceProjectBullets(project: Project): Promise<EnhancedBullet[]> {
  const client = getClient()

  const bulletList = project.bullets.map((b, i) => `${i + 1}. ${b}`).join('\n')

  const prompt = `Enhance the following resume bullet points for a project called "${project.name}".
